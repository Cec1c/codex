...
def assemble(archive, checksums, binary, host, output, upstream_version, version, target):
    ...
    digests = [line.split()[0] for line in checksums.read_text().splitlines()
    ...
    if windows:
        required.update({"codex-resources/codex-command-runner.exe",
    ...
    with tarfile.open(archive, "r:gz") as source:
        ...
        if (metadata.get("version"), metadata.get("target"), metadata.get("variant")) != (
                upstream_version, source_target, "codex"):
            raise ValueError("Upstream package metadata does not match requested release")
        ...
        with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as destination:
            ...
            for name, path in ((f"bin/codex{suffix}", binary),
            ...
            destination.writestr("package/codex-package.json", json.dumps(metadata, indent=2) + "\n")
