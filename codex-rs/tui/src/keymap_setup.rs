...
pub(crate) fn build_keymap_action_menu_params(
    ...
    let source = if custom_binding {
        keymap_setup_text("keymap-source-root-override", "Custom root override").fg(accent_color())
    } else {
        keymap_setup_text("keymap-source-default-keymap", "Default keymap").dim()
    };
    let header = vec![
        Line::from(keymap_setup_text("keymap-edit-shortcut-title", "Edit Shortcut").bold()),
...
pub(crate) fn build_keymap_replace_binding_menu_params(
    ...
    let label = action_label(&action);
    let header = vec![
        Line::from(keymap_setup_text("keymap-replace-binding-title", "Replace Binding").bold()),
        Line::from(vec![
            label.bold(),
            " · ".dim(),
...
pub(crate) fn build_keymap_conflict_params(
    ...
    SelectionViewParams {
        header: Box::new(
            Paragraph::new(vec![
                Line::from(keymap_setup_text("keymap-conflict-title", "Shortcut Conflict").bold()),
                Line::from(
                    keymap_setup_text_with_args(
