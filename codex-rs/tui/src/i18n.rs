...
// The runtime locale is fixed at startup. Cache static labels at each call site
// so upstream APIs that accept static text can participate in Fluent localization.
macro_rules! tr {
    ($key:literal, $english:literal) => {{
        static TEXT: std::sync::OnceLock<String> = std::sync::OnceLock::new();
        TEXT.get_or_init(|| $crate::i18n::global().text($key, None, || $english.to_string()))
            .as_str()
    }};
}
pub(crate) use tr;
...
macro_rules! tr_format {
    ($key:literal, $english:literal, $($name:ident = $value:expr),+ $(,)?) => {{
        ...
        $(args.set(stringify!($name), $name.to_string());)+
        $crate::i18n::global().text($key, Some(&args), || format!($english, $($name = $name),+))
    }};
}
