...
#[cfg(test)]
#[path = "ccu_theme_claude_tests.rs"]
mod claude_tests;
...
pub(crate) fn parse_theme(source: &str, theme_id: &str, seed: u64) -> Option<CcuTheme> {
    ...
    for value in [
        document.status_line.colors.progress_empty.as_deref(),
        document.status_line.colors.percent.as_deref(),
        document.status_line.colors.active_time.as_deref(),
        document.status_line.colors.permissions.as_deref(),
    ...
    if let Some(ui) = &document.interface {
        if ui.layout != "claude" {
            return None;
        }
        for value in [&ui.foreground, &ui.muted, &ui.user_background, &ui.rule] {
            parse_rgb(value)?;
        }
    }
...
impl CcuTheme {
    pub(crate) fn claude_layout(&self) -> bool {
        self.interface.is_some()
    }

    pub(crate) fn interface_style(&self, role: &str) -> Option<Style> {
        let ui = self.interface.as_ref()?;
        let value = match role {
            "foreground" => &ui.foreground,
            "muted" => &ui.muted,
            "userBackground" => &ui.user_background,
            "rule" => &ui.rule,
            _ => return None,
        };
    ...
    pub(crate) fn status_style(&self, role: &str) -> Option<Style> {
        let value = match role {
            ...
            "progressEmpty" => self.status_line.colors.progress_empty.as_ref()?,
            "percent" => self.status_line.colors.percent.as_ref()?,
            "activeTime" => self.status_line.colors.active_time.as_ref()?,
            "permissions" => self.status_line.colors.permissions.as_ref()?,
            _ => return None,
        };
        theme_color(value).map(|color| Style::default().fg(color))
    }
    ...
    pub(crate) fn progress_spans(&self, used_percent: u8) -> Vec<ratatui::text::Span<'static>> {
        use ratatui::text::Span;
        let used_percent = used_percent.min(100);
        let filled = (usize::from(used_percent) * self.status_line.progress_width + 50) / 100;
        let empty = self.status_line.progress_width.saturating_sub(filled);
        let style = self.status_style("progress").unwrap_or_default();
        vec![
            Span::styled("[", style),
            Span::styled(self.status_line.filled.repeat(filled), style),
            Span::styled(
                self.status_line.empty.repeat(empty),
                self.status_style("progressEmpty").unwrap_or(style),
            ),
            Span::styled("] ", style),
            Span::styled(
                format!("{used_percent}%"),
                self.status_style("percent").unwrap_or(style),
            ),
        ]
    }
