...
fn search_line(state: &PickerState, width: u16) -> Line<'_> {
    ...
    if state.query.is_empty() {
        resume_text("resume-search-placeholder", "Type to search")
...
fn toolbar_for_width(state: &PickerState, width: u16) -> Line<'static> {
    ...
    let spans = match state.toolbar_focus {
        ...
        ToolbarControl::Status => vec![
            ...
            toolbar_value(
                match state.status {
                    SessionStatus::Active => resume_text("resume-status-active", "Active"),
                    SessionStatus::Archived => resume_text("resume-status-archived", "Archived"),
                },
                /*active*/ true,
                /*focused*/ true,
            ),
...
fn sort_control_spans(state: &PickerState, compact: bool) -> Vec<Span<'static>> {
    ...
    if compact {
        return vec![
            resume_text("resume-sort-label", "Sort:").set_style(secondary_text_style()),
            toolbar_value(
...
fn filter_control_spans(state: &PickerState, compact: bool) -> Vec<Span<'static>> {
    ...
    if compact || state.filter_cwd.is_none() {
        return vec![
            resume_text("resume-filter-label", "Filter:").set_style(secondary_text_style()),
            toolbar_value(
...
fn render_expanded_session_details(
    ...
    let branch = format!(
        ...
        row.git_branch
            .as_deref()
            .unwrap_or(&resume_text("resume-no-branch", "no branch"))
    );
    ...
    vec![
        ...
        vec![
            "  │ ".set_style(secondary_text_style()),
            resume_text("resume-detail-conversation", "Conversation:")
                .set_style(secondary_text_style()),
        ]
        .into(),
    ]
...
fn expanded_detail_line(label: &str, value: &str, width: u16) -> Line<'static> {
    ...
    vec![
        "  │ ".set_style(secondary_text_style()),
        pad_to_display_width(label, LABEL_WIDTH).set_style(secondary_text_style()),
        "  ".set_style(secondary_text_style()),
...
fn render_empty_state_line(state: &PickerState) -> Line<'static> {
    if !state.query.is_empty() {
        ...
        {
            return vec![
                resume_text("resume-searching", "Searching…")
                    .set_style(secondary_text_style())
                    .italic(),
            ]
            .into();
