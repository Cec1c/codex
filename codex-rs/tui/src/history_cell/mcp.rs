...
fn mcp_auth_status_label(status: McpAuthStatus) -> String {
    match status {
        McpAuthStatus::Unknown => mcp_text("mcp-auth-unknown", "Unknown"),
...
pub(crate) fn empty_mcp_output() -> WebHyperlinkHistoryCell {
    let mut docs_line = HyperlinkLine::new(Line::from(vec![
        "    ".into(),
        mcp_text("mcp-see-docs-prefix", "See the").into(),
        " ".into(),
    ]));
    docs_line.push_span(
        mcp_text("mcp-docs-label", "MCP docs").underlined(),
        Some("https://developers.openai.com/codex/mcp"),
    );
    docs_line.push_span(
        format!(" {}", mcp_text("mcp-see-docs-suffix", "to configure them.")).into(),
        /*destination*/ None,
    );
