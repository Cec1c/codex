...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        if let Some(theme) = crate::ccu_theme::active().filter(|theme| theme.claude_layout()) {
            return crate::ccu_welcome::WelcomeCard {
                version: self.version,
                model: &self.model,
                reasoning: self.reasoning_label(),
                directory: &self.format_directory(Some(usize::from(width.saturating_sub(6)))),
                yolo_mode: self.yolo_mode,
                fast: self.show_fast_status,
            }
            .lines(width, theme);
        }
        ...
        let model_spans: Vec<Span<'static>> = {
            ...
            spans.push(Span::styled(
                ...
                ccu_theme
                    ...
                    .unwrap_or_else(|| Style::default().fg(accent_color())),
            ));
            spans.push(" ".dim());
